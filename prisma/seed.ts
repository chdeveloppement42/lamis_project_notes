...
// Self-contained: no dependency on src/ so this works in both dev and delivery packages.
const hashPassword = promisify(bcrypt.hash);

dotenv.config();

const pool = new Pool({
  connectionString: process.env.DATABASE_URL || 'postgresql://postgres:0000@localhost:5432/immo_lamis?schema=public',
});
...
async function main() {
  ...
  const permissionsData = [
    ...
    { action: 'manage:admins', description: 'Créer / Modifier / Suspendre des comptes administrateurs et réinitialiser les mots de passe' },
  ...
  const password = await hashPassword('0000', 10);
